...
function configure_linux() {
  ...
  declare -a services_to_stop=("avahi-daemon" "avahi-daemon.socket" "dnscrypt-proxy.socket" "systemd-resolved" "cups")
  myecho -e "\nConfiguring services to be stopped ... \n"
	for srv2stop in "${services_to_stop[@]}"; do
    $SYSTEMCTL_BIN is-active $srv2stop --quiet 2>/dev/null
    [[ $? -eq 0 ]] || $SYSTEMCTL_BIN stop $srv2stop --quiet 2>/dev/null
    $SYSTEMCTL_BIN is-enabled $srv2stop --quiet 2>/dev/null
    [[ $? -eq 0 ]] || $SYSTEMCTL_BIN disable $srv2stop --quiet 2>/dev/null
	done;
  ...
  if [ $? -eq 1 ]; then myecho -e "[\e[91m!\e[0m] No $DNSCRYPT_PROXY_USER user found.\n"; exit 1; fi
  declare -a dcdirs=("/var/cache/dnscrypt-proxy" "/var/log/dnscrypt-proxy")
  for dcdir in "${dcdirs[@]}"; do
    dcdir_u=$(stat -c "%U" $dcdir)
    dcdir_g=$(stat -c "%G" $dcdir)
    chown -R $dcdir_u:$dcdir_g $dcdir && \
    chown -R $dcdir_u:$dcdir_g $dcdir/*
  done
  # while true ; do
  ...
  if [ ! -d $TORO2_TOR_DATADIR ]; then
    ...
    chmod -R 775 $TORO2_TOR_DATADIR
  fi
  usermod -a -G toro2 root

  if [[ $foruser != "root" ]]; then
    usermod -a -G toro2 `who|awk '{print $1}'`
